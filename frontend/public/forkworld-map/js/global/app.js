...
(async function boot() {
  ...
  function createInner() {
    ...
    for (const w of REG.worlds) {
      ...
      if (EXTERNAL_AGENTS) spr.setAlpha(0);
  ...
  function postPlazaState() {
    if (!EXTERNAL_AGENTS || window.parent === window || !scene || !cam) return;
    const now = Date.now();
    if (now - lastPlazaPost < 50) return;
    lastPlazaPost = now;
    ...
    for (const id in envoys) {
      const env = envoys[id];
      const x = rect.left + (env.sprite.x - view.x) * scaleX;
      const y = rect.top + (env.sprite.y - view.y) * scaleY;
      payload[id] = {
        x,
        y,
        scale: (env.sprite.displayHeight * scaleY) / 58,
        rotation: env.sprite.rotation,
        flipX: env.sprite.flipX,
        visible: x > -60 && x < rect.width + 60 && y > -80 && y < rect.height + 80,
        visiting: env.visiting,
      };
    }
    window.parent.postMessage({
      type: "forkworld:plaza-state",
      envoys: payload,
